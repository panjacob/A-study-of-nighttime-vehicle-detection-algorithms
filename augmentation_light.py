...
def augment_images(src, dst, i):
    for x in tqdm(os.listdir(src)):
        ...
        if extension != 'png':
            ...
            # xml_text = open(os.path.join(src, f'{filename}.{extension}'), "r").read()
            # start = xml_text.find('</filename>') - 4
            # new_xml_text = xml_text[:start] + f'_aug{i}' + xml_text[start:]
            # f = open(os.path.join(dst, filename_xml), "w", encoding='UTF-8')
